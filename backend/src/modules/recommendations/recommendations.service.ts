...
export class RecommendationsService {
  ...
  indexAsset(asset: AssetDocument): void {
    ...
    this.httpService
      .post(`${this.aiServiceUrl}/index-asset`, payload)
      .subscribe({
        next: () => this.logger.log(`Indexed asset: ${String(asset._id)}`),
        error: (err) => {
          const status = err?.response?.status ?? err?.status;
          if (status === 503) {
            this.logger.debug(
              `Qdrant offline — skipped indexing asset ${String(asset._id)}`,
            );
          } else {
            this.logger.warn(
              `Failed to index asset ${String(asset._id)}: ${err?.message}`,
            );
          }
        },
      });
    ...
    this.httpService.delete(`${this.aiServiceUrl}/index/${assetId}`).subscribe({
      ...
      error: (err) => {
        ...
        if (status !== 503 && status !== 404) {
          this.logger.warn(
            `Failed to remove index for ${assetId}: ${err?.message}`,
          );
        }
      },
    });
  ...
  async getAssetRecommendations(
    ...
    try {
      ...
      const ids: string[] = (res.data?.similar ?? []).map(
        (s: any) => s.asset_id,
      );
      if (!ids.length)
      ...
      if (
        ...
        this.logger.warn(
          `AI service error for asset ${assetId}: ${err?.message}`,
        );
      }
      return this._fallbackAssetRecommendations(assetId, limit);
    }
  ...
  async getUserRecommendations(
    ...
    try {
      ...
      const ids: string[] = (res.data?.recommendations ?? []).map(
        (r: any) => r.asset_id,
      );
      ...
      if (
        ...
        this.logger.warn(
          `AI service error for user ${userId}: ${err?.message}`,
        );
      }
      return this._fallbackUserRecommendations(limit);
    }
  ...
  async reindexAll(): Promise<{ indexed: number; message: string }> {
    try {
      ...
      const res = await firstValueFrom(
        this.httpService.post(`${this.aiServiceUrl}/index-batch`, {
          assets: batch,
        }),
      );
  ...
  private _fallbackUserRecommendations(
    limit: number,
  ): Promise<AssetDocument[]> {
    return this.assetModel
      .find({ status: 'published' })
