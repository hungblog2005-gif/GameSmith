 ...
 */
const WEIGHTS = {
  downloads:    0.30,  // Total all-time download count
  rating:       0.25,  // Bayesian-weighted rating quality
  recentDL:     0.20,  // Downloads in the last 7 days (trending signal)
  views:        0.10,  // Total view count (lighter — easier to inflate)
  reviews:      0.10,  // Number of written reviews (social proof)
...
export class FeaturedScoreService {
   ...
   */
  async runScoreJob(featuredTopN = 10, trendingTopM = 10): Promise<ScoreJobResult> {
  ...
  private async _recomputeAllScores(): Promise<number> {
    ...
    await this.assetModel.aggregate([
      ...
      {
        $addFields: {
          _recentDownloads: { $ifNull: [{ $arrayElemAt: ['$_recentDLData.total', 0] }, 0] },
      ...
      {
        $unset: [
          '_recentDLData', '_recentDownloads', '_dlCount', '_viewCount',
          '_reviewCount', '_avgRating', '_ageMs',
          '_sDownloads', '_sViews', '_sRecentDL', '_sReviews', '_sRating', '_sFreshness',
        ],
      },
  ...
  private async _applyFeatured(ids: string[]): Promise<void> {
    await Promise.all([
      this.assetModel.updateMany({ status: 'published' }, { $set: { featured: false } }),
    ])
    if (ids.length > 0) {
  ...
  private async _applyTrending(ids: string[]): Promise<void> {
    await this.assetModel.updateMany({ status: 'published' }, { $set: { isTrending: false } })
    if (ids.length > 0) {
      await this.assetModel.updateMany(
        { _id: { $in: ids } },
        { $set: { isTrending: true } },
  ...
  async getScoreLeaderboard(limit = 20) {
    return this.assetModel
      ...
      .sort({ featuredScore: -1 })
      .limit(limit)
      .select('title featured isTrending featuredScore recentDownloads stats ratings createdAt')
      .lean()
  }
}
